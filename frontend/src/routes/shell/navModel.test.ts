...
describe('buildRailGroups', () => {
  ...
  it('groups Workspace (Assistant/Search/Documents) and Administration (Sources/Audit/Admin)', () => {
    const groups = buildRailGroups(DISCOVERED_NAV, DISCOVERED_ROUTES);
    const workspace = groups.find((g) => g.label === 'Workspace');
    const admin = groups.find((g) => g.label === 'Administration');
    expect(workspace?.links.map((l) => l.to)).toEqual(['/', '/search', '/documents']);
    expect(admin?.links.map((l) => l.to)).toEqual(['/sources', '/audit', '/admin']);
  });
