...
accounting; ``message_id`` is DEFERRABLE INITIALLY DEFERRED because the row is
written in the SAME transaction that inserts the assistant message (the FK must
validate at commit, not at flush — the ``0026`` lesson). ``run_id`` is reserved
for headless-run / sub-agent grouping (no FK yet, mirroring
``tool_invocations.run_id``).

Indexes: ``(tenant_id, session_id)`` (per-chat spend) and
``(tenant_id, created_at)`` (time-window analytics), each tenant-leading
(INV-1), plus the mixin's plain ``tenant_id`` index.
...
def upgrade() -> None:
    op.create_table(
        ...
        # The assistant message the answer persisted as. Deferred: written in
        # the same transaction that inserts the message (validate at COMMIT).
        sa.Column(
            "message_id",
            sa.Uuid(as_uuid=True),
            sa.ForeignKey(
        ...
        # The REQUESTED model id (matches messages.model) so a per-tenant
        # ``provider:`` id stays attributable to its provider config.
