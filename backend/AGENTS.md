...
- **The execution context is framework-supplied** (ADR-0019 §4): connector code never reads the vault, Lumen's DB, or mutable module state — it receives an already-authenticated, egress-guarded client on `ConnectorRun` and a frozen `AclMappingContext`. Pinned structurally by the conformance kit. (An external *source* that happens to be a SQL database is a normal vendor boundary; the prohibition is `app.db`, not SQL.)
- **Every connector package must pass `tests/test_connector_conformance.py`** (rules in `tests/conformance/`). The suite is parametrized over the real registry *and* checks enrollment from disk — registry discovery silently **skips** a `CONNECTOR` that fails the runtime protocol, so a malformed new connector would otherwise vanish instead of failing. A new connector ships with its harness or the suite fails.
