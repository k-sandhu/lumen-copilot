...
:func:`enqueue_ingestion`; the connector **sync** task (#20, ADR-0009 §5):
...
the search **index sync** task (ADR-0010 §5, epic #189): make the OpenSearch
...
and the artifact **retention janitor** (#208 §6): a stub task that purges expired
agent/run artifacts (:func:`purge_expired_artifacts`) — defined + testable here,
its periodic scheduling deferred to E-Sched. Each enqueue helper is the single
enqueue point for its task.
"""

from app.tasks.artifact_retention import purge_expired_artifacts
