...
4. **Breakpoints & routing hints (amended by [#448](https://github.com/k-sandhu/lumen-copilot/pull/448), implementing [#411](https://github.com/k-sandhu/lumen-copilot/issues/411)):** the LiteLLM upgrade originally named as a prerequisite is **withdrawn — the pin (1.55.9) stays**. Validation showed both directive shapes are pure pass-through on the pinned version's OpenRouter adapter: `cache_control` content-parts ride the message list unmodified and `extra_body.prompt_cache_key` rides the request body (live-verified against a real Anthropic route: nonzero cache reads on turn 2). The gateway therefore decorates the serialized wire itself, post-`_to_wire_messages`; `cache_control_injection_points` is re-evaluated whenever LiteLLM is next upgraded, not before. The **route-capability map lives inside `llm/`, keyed off the resolved route the gateway already receives per call — (model id, api_base)**: `anthropic-style` → an explicit `cache_control` mark on the stable prefix boundary (message 0) plus a **moving mark on the last cacheable block** (each call writes its full prefix; the next append-only call reads it); `openai-style` → no marks, pass `prompt_cache_key=session_id`; **any custom `api_base`** (anything but the known OpenRouter host) → **omit every cache directive, fail-safe** — a route's model NAME never earns directives its HOST wasn't proven to accept. `ModelRoute` stays capability-free: duplicating the capability above `llm/` would leak provider categories into `services/`; the gateway is the single resolver. Each category keeps a gateway *payload* test (decorated, steered, bare). The kill-switch `CHAT_PROMPT_CACHE_ENABLED` is process-static (read at startup).
