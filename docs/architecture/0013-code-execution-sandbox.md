...
**Amendment ([#509](https://github.com/k-sandhu/lumen-copilot/issues/509), 2026-08-04) — the transitive-deny hole is CLOSED; the mirror is now possible but is still a deployment act.** Two of the three gaps the note above admits are fixed, and the third is narrowed rather than closed, so this paragraph states each separately instead of leaving one qualifier covering all of them.

*Closed — deny now applies to the RESOLVED set.* The runner performs the `pip download`, so it is the only component that ever sees what resolution produced; the wheelhouse it fetches **is** the install manifest, because `pip install --no-index --find-links` takes exactly what is in it. The tenant deny list is therefore forwarded to the runner on the execute request and re-applied to every wheel in that directory, matched on the PEP-503 canonical name read from the PEP-427 filename. A denied distribution admitted as a dependency of an allowed one now refuses the whole run — deny `urllib3`, allow `requests`, request `requests`, and the run is refused rather than silently installing `urllib3`. The check runs **before** any wheel is staged into the container: a denied artefact copied in and then refused has already crossed the boundary. Refusing the run rather than dropping the offending wheel is deliberate — removing one distribution from a resolved set produces a broken install, and handing the model a half-working environment is worse than a clear refusal.

*Closed — the audit records what actually ran.* `code_runs.resolved_packages` (migration `0043`) persists one entry per installed wheel: name, version, and the sha256 of the artefact as fetched. `requested_packages` records only what the model asked for, so a run that pulled in thirty distributions audited as one, and "did any run install the distribution later found to be malicious?" was unanswerable. The column is **nullable**: `NULL` means the run predates it, which is a different fact from `[]` ("this run installed nothing").

*Half-closed — verification.* `SANDBOX_PACKAGE_INDEX_URL` supplies the **allowlisted mirror** half of this section's "admin-allowlisted, hash-pinned internal mirror" clause: it pins where wheels come from, replacing the default index rather than adding to it, so a configured mirror is exclusive. Unset, the fetch is the default public index — unchanged.

**The hash-pinned half is still NOT implemented.** A first attempt shipped a `SANDBOX_PACKAGE_HASH_FILE` knob passing `--require-hashes --requirement <lockfile>` alongside the model's packages as command-line arguments. Verified against real pip, that can never succeed: an unpinned CLI requirement fails with *"all requirements must have their versions pinned with =="*, a pinned one with *"Hashes are required in --require-hashes mode"* — a command-line requirement cannot carry a `--hash`. Every install-bearing run would have failed the moment an operator enabled it, and since both pip messages contain "hash", the failure was reported as a supply-chain verification event: a false alarm on top of an outage. The knob was **removed** rather than left as a trap. Doing it properly means resolving the request against the lockfile and passing the resulting closure as a requirements file with no CLI packages at all; that is tracked separately.

The sha256 recorded per artefact does not substitute for pre-pinning — it cannot prove a wheel was the intended one — but it makes a run reproducible and lets an operator answer the question after the fact.

*Also:* admitting an install under the `*` allow-all grant (ADR-0020 §3) now emits an ops warning naming the distribution, because with `*` set the model chooses any public package name and that posture was previously visible only in a policy row nobody reads.
