...
*Narrowed, not closed — verification.* Two knobs now exist so a deploy can satisfy this section's "admin-allowlisted, hash-pinned internal mirror" clause: `SANDBOX_PACKAGE_INDEX_URL` pins where wheels come from (`--index-url` replaces the default rather than adding to it, so a pinned mirror is exclusive), and `SANDBOX_PACKAGE_HASH_FILE` points at a requirements file of `--hash=` lines, under which pip runs with `--require-hashes` and refuses any artefact — transitive ones included — whose digest is not listed. A hash mismatch is reported with its own reason so an operator can tell a supply-chain event from a flaky network. **With neither set the fetch is unchanged: the default public index, no hashes.** The mechanism is implemented; standing up the mirror and producing the lockfile are deployment acts, and this section will not claim the clause is satisfied until a deploy actually does both. The sha256 recorded per artefact does not substitute for pre-pinning — it cannot prove the wheel was the intended one — but it makes a run reproducible and lets an operator answer the question after the fact.
