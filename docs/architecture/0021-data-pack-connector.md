...
5. **Install scope is per-user (owner-scoped), as every source is today.** Ingested pack content stays tenant- and owner-scoped, deny-by-default (INV-1/INV-2), and the Sources grid shows it under the existing "Owner only" `PermissionPill`. **Tenant-wide provisioning is deliberately deferred**, not rejected: a firm wanting every accountant to hold the New York pack is a real want, but it needs a source-sharing model that does not exist yet, and inventing one here would couple a content decision to a permissions decision. Because the catalog is server-owned (§2), adding tenant provisioning later is additive — the pack identity does not change, only who a `sources` row belongs to.
...
- **Delivery follows the ADR-0008 M2 shape:** serialized prep (this ADR → catalog move → `/sources` + `/source-catalog` contract change → `SourceType` migration) then parallel build (datapack connector BE ‖ catalog browse FE), per [ADR-0006](0006-contract-first-parallel-implementation.md).
