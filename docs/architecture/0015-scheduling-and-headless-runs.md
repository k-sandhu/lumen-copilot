...
5. **Audit each run (INV-6).** `run.started` and `run.finished` audit events bracket every run (new actions, additive to the [spec 0004](../specs/0004-security-and-domain-invariants.md) §2.4 taxonomy), plus the per-retrieval `retrieval.query` and `answer.generated` events the runtime already emits. A scheduled run that has been created and is executing uses the **owner** as actor because the run acts on that user's behalf, tagged as schedule-triggered in metadata — so the trail shows *who* the run ran as and *why it fired*. This execution-principal attribution does **not** apply to a pre-run dispatcher denial: if the scheduler itself attempts to enqueue a now-hidden/non-runnable assistant, the initiating actor is `system` (`actor_id = NULL`, `source_origin = system`). The schedule owner remains execution/configuration context and must not be fabricated as the person who made that denied attempt.
...
- **Delivery v1 = in-app run inbox + digest.** A completed run lands in an **in-app inbox** (a dedicated `run_deliveries` table, owner-scoped + RLS) with its `summary` and a link (`run_id`) to the full cited transcript; an opt-in **digest** batches a schedule's low-urgency runs into one periodic in-app notification, drained by a `lumen.build_run_digests` celery-beat sweep (reusing the #236 Beat wiring). **External channels (email / Slack) are deferred** — they are outbound egress and cross into **T2-ish** territory ([spec 0004](../specs/0004-security-and-domain-invariants.md) §2.5); v1 stays read-before-write with **no external send** (each future channel would be its own `connectors/` module + a new AGENTS.md §6 boundary row). Every delivery is audited (`run.delivered` / `run.digest_sent` / `run.delivery_read`, INV-6); a produce failure is a queryable `failed` row, never a silent drop. **Landed in [#238]** (F-SCHED-4): the hook lives in [runs_service.execute_run](../../backend/app/services/runs_service.py) → [run_delivery_service](../../backend/app/services/run_delivery_service.py), surfaced by `GET /run-deliveries` + `POST /run-deliveries/{id}/read`.
- **Escalation (E7-5) = the `escalated` status.** On ambiguity / missing data / unrecoverable tool failure, the run ends `status=escalated` (a first-class terminal, distinct from `failed`) with the reason in `error`, **notifies the owner** (via the same inbox), and offers **resume / cancel / reroute**. Escalation detail — the triggers and the human-handoff UX — is handed to **[#239]** (F-SCHED-5). This ADR fixes only the *shape*: `escalated` is a run status, not a silent failure.
  - **Landed in [#239]:** the triggers are classified from the run's typed `error` in `services/runs_service.execute_run` (a pure `domain/escalation` classifier): a human-decidable failure (`ambiguous` / `missing_required_input` / `restricted_data` / `tool_failed` / `approval_required`, incl. the runtime's `forbidden` / `approval_denied` aliases) routes the terminal to `escalated` and audits `run.escalated`; deny-by-default keeps every other failure `failed`. A **transient** dependency fault (`dependency_unavailable` / `model_unavailable`) instead re-queues + retries with bounded backoff (`RUN_MAX_RETRIES` / `RUN_RETRY_BACKOFF_SECONDS`) **before** any terminal (§5). The handoff is `POST /runs/{id}/resume|cancel|reroute` (owner-scoped: cross-tenant / non-owned → 404, not-escalated → 409, INV-8), each audited (`run.resumed` / `run.cancelled` / `run.rerouted`); `reroute` reassigns the run's `owner_id` (its execution principal), so a rerouted run retrieves only what the *new* owner may (INV-2). No migration: `escalated` already exists and cancel reuses `error` (code `cancelled`). Delivery reuses the audited status as the in-app notification until [#238] lands the inbox delivery.
...
- Config via `core/config.py` (`pydantic-settings`): RedBeat key prefix / lock timeout, the per-tenant run concurrency + rate-window caps, retry backoff/cap, and the digest sweep interval (`RUN_DIGEST_INTERVAL_SECONDS`, #238 — a static `beat_schedule` entry firing `lumen.build_run_digests`). No secrets beyond the existing Redis URL. The `worker` `imports` gains `app.tasks.run_digest` alongside `run_assistant`/`scheduler`.
