...
- **No part of OD-4 remains "ask, don't default."** Items marked `revisit-at-implementation` (the RLS backstop — **now implemented, [#17]**; the explicit-grants permission filter — **now implemented, [#18]** (the grant HTTP API/UI + `group`/`role` principals remain a follow-up); connector-ACL mirroring, SSO/OIDC wiring, audit hash-chain — still sequenced later) are **decided**; each remaining one reconfirms against this spec when claimed.
