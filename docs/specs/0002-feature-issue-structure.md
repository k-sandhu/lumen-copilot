...
>
> **Status: ADOPTED in practice (2026-06-17).** The layer model (Epic/Feature/Cross-cutting/Spike/Task), the `type:*`/`area:*`/`size:*`/`risk:*` label set, and the 6-state board all landed via the fleet-tooling chores ([#7](https://github.com/k-sandhu/lumen-copilot/issues/7), [#9](https://github.com/k-sandhu/lumen-copilot/issues/9)) and are how every issue since M2 is cut and claimed. **What diverged from this draft:** (1) **milestones are thematic waves** (`M0 Foundations`, `M2 Trust Surfaces`, `M3 Agents & Extensibility`, `M2.5 Stabilization`…) rather than §7.3's numbered `M0…M5` iterations — the epic lives as a label / issue back-reference, not a milestone, as recommended, but the wave names are descriptive not numeric; (2) the provisional CC-1..CC-12 list in §5.1 was **superseded** by the reconciled table in [consolidated-structure.md](../product/consolidated-structure.md) §5 (those CCs are built and largely Done — see [#17](https://github.com/k-sandhu/lumen-copilot/issues/17)–[#26](https://github.com/k-sandhu/lumen-copilot/issues/26)); (3) the §12 open questions were resolved in session (vendor-neutral `user-stories.md` is canonical; Planner cuts Features by hand rather than mass-generating from stories). The §11 rollout phases A–D are **done**; §10's open decisions all closed (OD-1..OD-5 — see [0001-open-decisions.md](0001-open-decisions.md)). Treat §§3–8 as the live contract; §§10–12 as historical. **Last reviewed:** 2026-07-02.
