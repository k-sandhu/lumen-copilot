...
**Architecture record:** [ADR-0019](../architecture/0019-connector-sdk-and-oauth.md) (from spike [#290](https://github.com/k-sandhu/lumen-copilot/issues/290)) decides the OAuth flow, the concrete ACL-mirror contract, change detection, and the SDK shape. **This slice is governed by the *unchanged* mission filters (§2) and risk tiers ([spec 0004](0004-security-and-domain-invariants.md) §2.5)** — *retrieval* over connector content is T0; the source-management writes (create / connect / reconnect / disconnect, and on-demand sync) are **T1** — admin-gated, audited, reversible — with the provider consent screen as the explicit human authorization of the connect (the ADR-0009 §5 classification, unchanged); deny-by-default is preserved (unmapped, domain-restricted, or stale ACLs deny).
